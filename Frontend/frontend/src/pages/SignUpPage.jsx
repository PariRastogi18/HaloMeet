...
export default function SignUp() {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Auto-login the user with the accessToken
  ...
  return (
    <div className="min-h-screen bg-black dark:bg-gray-900 text-white font-sans selection:bg-purple-600 selection:text-white">
      ...
      <Navbar />
      {/* Decorative Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 sm:w-125 sm:h-125 bg-purple-600/10 rounded-full blur-[120px] pointer-events-none" />

      {/* Main Container */}
      <div className="w-full max-w-md bg-zinc-950/60 backdrop-blur-xl border border-zinc-800/80 rounded-2xl p-5 sm:p-8 shadow-2xl relative z-10 my-8 mx-auto">
        {/* Logo and Header */}
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold tracking-tight bg-linear-to-r from-white to-purple-400 bg-clip-text text-transparent">
            Create your HaloMeet Account
          </h2>
          <p className="text-zinc-400 text-sm mt-2">
            Get started with free high-definition video calls
          </p>
        </div>
        ...
        <form onSubmit={handleSignUp} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-white placeholder-zinc-600 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all text-sm"
              required
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-800/50 disabled:text-zinc-400 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-purple-600/10 active:scale-[0.98] mt-2 text-sm"
          >
